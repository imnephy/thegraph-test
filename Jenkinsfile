          ...
          stages {
            stage('Approve') {
              input {
                ...
                ok 'Yes'
                submitter 'ismirnov, dsemyonov, alukashenko, nbobkov'
              }

              environment {
                LOKI = credentials('LOKI')
                DOCKER = credentials('DOCKER')
                SSH_PROFILE = 'romannft'
                COMPOSE_PROJECT_NAME = 'roman-nft'
                FOLDER = 'frontend'
                DOMAIN = 'dropsmint.net'
                PRODUCTION_URL = 'https://dropsmint.net'
                ENV_FILE = '.env.production'
                CLIENT_EMAIL = 'artema@unistory.app'
              }
